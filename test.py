...
# CONFIG
# ============================================================
pdf_dir = r"C:\Analysis\input"
output_dir = r"C:\Analysis\Report1\extracted_images"
legend_dir = r"C:\Analysis\Report1\cropped_legends"
report_path = r"C:\Analysis\output\report.docx"
log_csv_path = r"C:\Analysis\logs_Csv\process_log.csv"

OLLAMA_API = "http://localhost:11434/api/generate"
MODEL_NAME = "llama3.2-vision:latest"  # vision model
...
def _check_port(host="localhost", port=11434, timeout=20):
    try:
...
# ============================================================
## ============================================================
# LEGEND PARSER VIA OCR + LLM (text only)
# ============================================================
def parse_legends_with_llm(legend_entries):
    ...
    for kpi in KPI_LIST:
        ...
        # --- 1) OCR the cleaned legend image to TEXT ---
        try:
            img = Image.open(legend_path)
            # psm 6 = assume a block of text, works well for legends
            ocr_text = pytesseract.image_to_string(img, config="--psm 6")
            ocr_text = ocr_text.strip()
            if not ocr_text:
                print(f"   ⚠️ OCR returned empty text for {kpi} legend: {legend_path}")
                continue
        except Exception as e:
            print(f"   ⚠️ Could not OCR legend image for {kpi}: {e}")
            continue

        # --- 2) Build a TEXT prompt (no images field) ---
        base_prompt = _build_vision_prompt_for_kpi(kpi)
        prompt = (
            base_prompt
            + "\n\nLegend OCR text (exactly as read from the image):\n"
            + "-----------------------------\n"
            + ocr_text
            + "\n-----------------------------\n"
            + "Use ONLY these percentages to fill the JSON fields."
        )

        payload = {
            "model": MODEL_NAME,      # llama3.2-vision:latest works fine with text-only
            "prompt": prompt,
            "stream": False,          # simpler parsing
            "options": {
                "num_ctx": 4096,
                "num_predict": 256,   # keep answer short
                "temperature": 0.0,
            },
        }

        try:
            # Text-only → much faster; 60s overall timeout is plenty
            r = requests.post(OLLAMA_API, json=payload, timeout=60)
            if not r.ok:
                print(f"❌ LLM API error for {kpi}: {r.status_code} — {r.text}")
                continue

            raw = r.json().get("response", "").strip()
            if not raw:
                print(f"   ⚠️ Empty response from {MODEL_NAME} for {kpi}.")
                continue

            raw_chunks.append(f"### {kpi}\n{raw}")

            # ---------- existing JSON cleaning/parsing logic ----------
            cleaned = raw.strip()
            ...
            try:
                ...
                print(f"❌ Failed to parse JSON for {kpi}. Raw kept in log.")
                continue
            ...
            print(f"❌ Read timeout while querying {MODEL_NAME} for {kpi} (text mode).")
            continue
        except Exception as e:
